...
function deployArtifact() {
  ...
  if [[ -d "${POM_FOLDER}/target/" && $(ls "${POM_FOLDER}/target/." | grep "\-${VERSION}.jar" | wc -l | xargs) -gt 0 ]]; then
    ls "${POM_FOLDER}/target/." | grep "\-${VERSION}.jar" | xargs -L1 -I{} mvn deploy:deploy-file -q -s ./.m2/settings.xml -DrepositoryId=github -Dfile="${POM_FOLDER}/target/"{} -DpomFile="${POM_PATH}" -Durl=https://maven.pkg.github.com/navikt/fp-felles
  else
