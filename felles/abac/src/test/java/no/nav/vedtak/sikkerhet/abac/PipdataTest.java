...
class PipdataTest {
    ...
    void roundtrip_compatible1() {
        var pip = new PseudoPip(Set.of("0000000000000", "2222222222222"), PipFagsakStatus.UNDER_BEHANDLING.name(),
            PipBehandlingStatus.UTREDES.name());
        var json = DefaultJsonMapper.toJson(pip);
        var roundtrip = DefaultJsonMapper.fromJson(json, AbacPipDto.class);
    ...
    void roundtrip_compatible2() {
        var pip = new AbacPipDto(Set.of(new PipAktørId("0000000000000"), new PipAktørId("2222222222222")), PipFagsakStatus.UNDER_BEHANDLING,
            PipBehandlingStatus.UTREDES);
        var json = DefaultJsonMapper.toJson(pip);
    ...
    void fra_kilde_sak() {
        var json = """
            ...
            """;
        var roundtrip = DefaultJsonMapper.fromJson(json, AbacPipDto.class);
        assertThat(roundtrip.aktørIder().stream().map(PipAktørId::getVerdi).collect(Collectors.toSet())).containsAll(
            Set.of("0000000000000", "2222222222222"));
