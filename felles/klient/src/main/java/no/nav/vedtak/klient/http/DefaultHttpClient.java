...
public final class DefaultHttpClient {
    ...
    private static DefaultHttpClient PROXY_CLIENT;

    private final HttpClient httpClient;

    private DefaultHttpClient() {
        this(HttpClient.Builder.NO_PROXY);
    }

    private DefaultHttpClient(ProxySelector proxySelector) {
    ...
    public static synchronized DefaultHttpClient proxyclient() {
        var inst = PROXY_CLIENT;
        if (inst == null) {
            inst = new DefaultHttpClient(ProxyProperty.getProxySelectorIfFSS());
            PROXY_CLIENT = inst;
        }
        return inst;
    }
    ...
    private HttpResponse<byte[]> doSendExpectBytearray(HttpRequest request) {
        try {
            ...
            throw new IntegrasjonException("F-157392", "InterruptedException ved kall mot endepunkt", HttpURLConnection.HTTP_CLIENT_TIMEOUT, e);
        }
    }
